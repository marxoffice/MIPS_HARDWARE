...
|    时间     |  人员  |                          工作内容                          |
| :---------: | :----: | :--------------------------------------------------------: |
| 12:00-23:00 | 朱海龙 | 修改分支预测机制，将延迟槽指令先运行，并重新测试branch指令 |
...
解决方案：对于beq类型的指令，由于我们已经实现了分支预测机制，可以在D阶段预测，E阶段判定预测是否正确，所以在load时，先进行一个lwstall，然后在alu阶段进行了一个前推，不会影响我们的判定预测是否正确的模块。
